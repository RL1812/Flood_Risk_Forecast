...
def extract_alert_records(date_list, cache):
    """
    Parses cached API records and extracts structured flood alert records.
    """
    alerts = []
...
def fetch_flood_alerts(
    ...
    if api_key:
        ...
        print("[!] Warning: No DATA_GOV_SG_API_KEY found. Proceeding without API key.", flush=True)
    ...
    date_list = [(start_dt + datetime.timedelta(days=i)).isoformat() for i in range(total_days)]
    print(f"[*] Querying {total_days} dates ({start_date} to {end_date})...", flush=True)
    ...
    if os.path.exists(cache_path):
        try:
            with open(cache_path, "r", encoding="utf-8") as f:
                cache = json.load(f)
            print(f"[*] Loaded {len(cache)} cached date queries from {cache_file}.", flush=True)
        except Exception:
            cache = {}
    ...
    for idx, d_str in enumerate(date_list, 1):
        ...
        if d_str in cache:
            ...
            while True:
                try:
                    resp = session.get(API_URL, params={"date": d_str}, timeout=15)
                    if resp.status_code == 200:
                        ...
                        print(f"[!] HTTP 429 (Rate Limit) on {d_str}. Sleeping for 10.5s...", flush=True)
                        time.sleep(10.5)
                    else:
                        print(f"[!] Warning: HTTP {resp.status_code} for {d_str}", flush=True)
                        cache[d_str] = []
                        break
                except Exception as ex:
                    print(f"[!] Connection exception on {d_str}: {ex}, retrying...", flush=True)
                    time.sleep(3)
        ...
        if idx % 50 == 0 or idx == total_days:
            print(f"    Progress: {idx}/{total_days} dates processed ({(idx/total_days)*100:.1f}%) | Alerts found so far: {alerts_found_count}", flush=True)
    ...
    print(f"[+] Saved structured flood alert records to: {output_json_path}", flush=True)

    return alerts


def main():
    parser = argparse.ArgumentParser(description="Fetch flood alert records across Singapore from Data.gov.sg API.")
    parser.add_argument("--start", "--start-date", dest="start_date", default="2024-01-01", help="Start date (YYYY-MM-DD)")
    parser.add_argument("--end", "--end-date", dest="end_date", default="2026-08-24", help="End date (YYYY-MM-DD)")
    parser.add_argument("--api-key", dest="api_key", default=None, help="Data.gov.sg API key (optional, can also be read from .env)")
    parser.add_argument("--delay", type=float, default=3.4, help="Pacing delay in seconds between queries (default: 3.4)")
    parser.add_argument("--cache-file", default="flood_alerts_cache.json", help="Cache JSON filename")
    parser.add_argument("--output-json", default="flood_alert_records.json", help="Output JSON filename")
    ...
    print("\n" + "=" * 70)
    print(f"FLOOD ALERTS SUMMARY ({args.start_date} to {args.end_date})")
    print("=" * 70)
    if not alerts:
        print("No flood alerts recorded in the specified date range.")
    else:
        for idx, alert in enumerate(alerts, 1):
            print(f"\nAlert #{idx}:")
            print(f"  • Date & Time:   {alert.get('datetime')}")
            print(f"  • Event:         {alert.get('event')}")
            print(f"  • Headline:      {alert.get('headline')} [Severity: {alert.get('severity')}, Urgency: {alert.get('urgency')}]")
            print(f"  • Status:        {alert.get('status')}")
            print(f"  • Location Desc: {alert.get('areaDesc')}")
            print(f"  • Coordinates:   Latitude {alert.get('latitude')}, Longitude {alert.get('longitude')}")
